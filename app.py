...
def generate_charts(profile_id):
    """Redraw the score-trend, section-breakdown and pie PNGs for a profile.

    Each chart is drawn inside its own try/except so that one chart failing
    to draw does not stop the others (or the mock test save) from working.
    """
    db = get_db()
    rows = db.execute(
